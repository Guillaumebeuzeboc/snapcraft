...
class TestPluginColconPlugin:
    ...
    def test_get_build_commands_core22(
        ...
        assert plugin.get_build_commands() == [
            ...
            "## Build command",
            "colcon build "
            f'--base-paths "{plugin._part_info.part_src_dir}" --build-base "{plugin._part_info.part_build_dir}" '
            f"--merge-install --install-base {plugin._part_info.part_install_dir}/opt/ros/snap "
            "--cmake-args -DCMAKE_BUILD_TYPE=Release "
            f"--parallel-workers {plugin._part_info._parallel_build_count}",
            "## Post build command",
    ...
    def test_get_build_commands_with_all_properties_core22(
        ...
        assert plugin.get_build_commands() == [
            ...
            "",
            "## Sourcing ROS ws in build snaps",
            'if [ -n "${ROS_DISTRO:-}" ] && [ -f "/snap/foo/current/opt/ros/${ROS_DISTRO:-}/local_setup.sh" ]; then',
    ...
    def test_get_build_commands_with_all_properties_core24(
        ...
        assert plugin.get_build_commands() == [
            ...
            'ros2 pkg list | (xargs rosdep resolve --rosdistro "${ROS_DISTRO}" || echo '
